...
#[cfg(not(all(target_os = "macos", feature = "system-audio-macos")))]
#[derive(Debug)]
pub struct SystemAudioCapture;
...
mod tests {
    ...
    #[test]
    fn system_audio_is_unsupported_without_feature() {
        // When the ScreenCaptureKit feature isn't enabled, we should deterministically return
        // UnsupportedSource instead of hitting any platform APIs.
        #[cfg(not(all(target_os = "macos", feature = "system-audio-macos")))]
        {
            let (tx, _rx) = mpsc::channel::<WriterCommand>();
            let control = Arc::new(RecordingControl::new());
            let err = start(tx, control).unwrap_err();
            assert!(matches!(err, super::super::RecordingError::UnsupportedSource));
        }
    }
}
