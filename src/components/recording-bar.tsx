...
export function RecordingBar({ onStop, onCancel, source = "Microphone" }: { onStop: (elapsed: number) => void; onCancel: () => void; source?: string }) {
  ...
  return (
    <div className="absolute left-1/2 -translate-x-1/2 bottom-6 z-50 flex items-center gap-4 bg-[var(--surface-card)] border border-[var(--border-subtle)] rounded-2xl shadow-xl px-5 py-3">
      ...
      <div className="w-[92px]" style={{ opacity: paused ? 0.4 : 1 }}>
        <Waveform n={18} live={!paused} color="var(--live)" height={24} width={92} seed={5} />
      </div>
